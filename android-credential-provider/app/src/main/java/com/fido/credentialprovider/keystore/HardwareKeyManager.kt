...
object HardwareKeyManager {
    ...
    sealed class KeyGenOutcome {
        data class Success(
            ...
            /**
             * true 代表這把金鑰其實未達硬體安全等級，只是因 [DiagnosticsGate.allowSoftwareKeyForInspection]
             * 診斷旗標而放行；該旗標只存在於 `poc` build flavor（見
             * `src/poc/.../keystore/DiagnosticsGate.kt`），`prod` flavor 恆為 false。
             */
    ...
    fun generate(alias: String, attestationChallenge: ByteArray): KeyGenOutcome {
        ...
        if (!hardwareLevelMet && !DiagnosticsGate.allowSoftwareKeyForInspection) {
        ...
        if (!hardwareLevelMet) {
            Log.w(
                ...
                "【PoC 診斷旗標開啟】偵測到非硬體安全等級（$detectedLevel），" +
                    "本應拒絕，但 DiagnosticsGate.allowSoftwareKeyForInspection=true，" +
